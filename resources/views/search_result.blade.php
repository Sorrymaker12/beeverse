        ...
        <div class="row row-cols-1 row-cols-md-4 g-4">
            @foreach ($users as $user)
                    ...
                    <div class="col">
                        <div class="card" style="height: 800px; width: 450px">
                            ...
                            <div class="card-body text-center">
                                ...
                                <h5 class="card-title">{{ $user->current_job }} at {{ $user->current_company }}
                                </h5>
                                ...
                                <div class="d-flex flex-row mb-3 justify-content-center">
                                    <a class="btn btn-primary mx-2" href="/profile/{{ $user->id }}" role="button"
                                        style="background-color: #89CFF0; border-color: #89CFF0">View
                                        Profile</a>
                ...
                @else
                    @foreach ($int as $i)
                        @if (Auth::check())
                            @if ($i->user_id == $user->id && $i->fow_id == $fow && $user->id != Auth::user()->id)
                                <div class="col">
                                    <div class="card" style="height: 800px; width: 450px">
                                        <a href="/profile/{{ $user->id }}">
                                            <img src="{{ asset('Storage/img/' . $user->profile_picture) }}"
                                                class="card-img-top" alt="..."
                                                style="height: 500px; background-size: auto; object-fit: cover; object-position: 100% 0"></a>
                                        <div class="card-body text-center">
                                            <h5 class="card-title">{{ $user->name }}</h5>
                                            <h5 class="card-title">{{ $user->current_job }} at
                                            ...
                                            @endforeach
                                            <div class="d-flex flex-row mb-3 justify-content-center">
                                                <a class="btn btn-primary mx-2" href="/profile/{{ $user->id }}"
                                                    role="button"
                                                    style="background-color: #89CFF0; border-color: #89CFF0">View
                                                    Profile</a>
                                                <form method="POST" action="/wish">
                            ...
                            @endif
                        @else
                            @if ($i->user_id == $user->id && $i->fow_id == $fow)
                                <div class="col">
